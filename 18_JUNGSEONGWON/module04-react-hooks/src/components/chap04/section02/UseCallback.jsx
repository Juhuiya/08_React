 ...
 * [useCallback이란 무엇인가?]
    ...
    * 메모이제이션 : 컴퓨터 프로그램이 동일한 계산을 반복해야 할 때, 
                  이전에 계산한 값을 메모리에 저장함으로써 동일한 계산의 
                  반복 수행을 제거하여 프로그램 실행속도를 빠르게 하는 기법이다. 
    ...
    const memoizedCallback = useCallback(() => {
     ...
     - 리렌더링 시 매번 새로 만들어지는 함수 인스턴스를 방지하고 싶을 때 
 */
import React,{ useState, useCallback } from "react";
//useCallback를 사용하지 않는 button
// React.memo : 함수를 React.memo로 래핑되면 React는 컴포넌트를 랜더링하고 결과를 메모라이징 한다.
const ButtonA = React.memo(({ onClick }) => {
  console.log(`props 변경으로 인해 ButtonA 렌더링`);
...
// value가 변경되면 buttonA는 렌더링되지만 buttonB는 렌더링 되지 않는다.
...
export default function UseCallback() {
  ...
  return (
    <div>
      <h2>UseCallback</h2>
      <input type="text" value={value} onChange={e => setValue(e.target.value)} placeholder="아무거나 입력해주세요." />
      <p>입력 값 : {value}</p>
      <p>카운트 : {count}</p>
