...
export default function UseMemoExample() {
  ...
  const [rederingCount, setRenderingCount] = useState(0);

  // 느린 계산 함수
  function slowFunction(num) {
    for (let i = 0; i < 700000000; i++) {} //일부러 지연
    console.log("느린 계산 수행 중...");
    return num * 2;
  }

  // useMemo를 사용하여 number가 변경될 때만 slowFunction을 재 실행
  ...
  return (
    <div>
      ...
      <hr />
      <p>두 배 값 : {doubleNumber}</p>
      <button onClick={()=> setRenderingCount(prev => prev +1)}
        style={{ marginTop: '60px' }}>{rederingCount}번 렌더링</button>
