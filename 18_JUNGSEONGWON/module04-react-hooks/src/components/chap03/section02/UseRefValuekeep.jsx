 ...
 * - useRef는 DOM 참조 뿐만 아니라, 컴포넌트가 재 랜더링되어도 값이 유지되는 저장소 역할도 한다.
 * - useRef에 저장된 값은 setState처럼 렌더링을 트리거하지 않고 유지된다.
 *
 * ref.current를 수정해도 컴포넌트는 재랜더링 되지 않는다.
 * > 리셋되지 않는 메모장과 같다.
 * > 최근 입력값, 이전 상태 값, 함수 호출 횟수 등을 기억할 때 유용하다.
 */
...
export default function UseRefValuekeep() {
  ...
  const handleChange = (event) => {
    ...
    countRef.current += 1; // 입력이 바뀔 때마다 countRef를 1 증가
    count += 1; // 일반 변수는 리렌더링 될때마다 초기화 된다.
