...
- useRef는 DOM 참조 뿐만 아니라, 컴포넌트가 재 렌더링되어도 값이 유지되는 저장소 역할을 한다.
- useRef에 저장된 값은 setState처럼 렌더링을 트리거하지 않고 유지된다.

ref.current를 수정해도 컴포넌트는 재랜더링되지 않는다.
> 리셋되지 않는 메모장과 같음
> 최근 입력값, 이전 상태값, 함수 호출 횟수 등을 저장하는데 유용하다.
*/
...
export default function UseRefValuekeep() {
  ...
  const handleChange = (event) => {
    ...
    countRef.current += 1; // 입력이 바뀔 때 마다 countRef를 1 증가
    count += 1; // 일반 변수는 리렌더링될 때 마다 초기화된다.
