...
- 리랜더링 시 매번 새로 만들어지는 함수 인스턴스를 방지하고 싶을 때때
*/
...
const ButtonA = React.memo(({ onClick }) => {
  console.log("props 변경으로 인해 Button A가 랜더링");
  return <button onClick={onClick}>UseCallback 안씀</button>;
});

// UseCallback을 사용한 버튼
const ButtonB = React.memo(({ onClick }) => {
  console.log("props 변경으로 인해 Button B가 랜더링");
  return <button onClick={onClick}>UseCallback 씀</button>;
});

// value가 변경되면 buttonA는 렌더링되지만 buttonB는 렌더링되지 않음
// useCallback을 사용하면 함수를 메모이제이션하여 함수를 재사용하기 때문이다.
// (콘솔 확인인)
