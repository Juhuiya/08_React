...
const TodoList = () => {
  ...
  const addTask = () => {
    ...
    if (newTask.trim()) {
      ...
      setTodos((prevTodos) => [...prevTodos, newTodo]); // 새 배열로 상태 업데이트
  ...
  // 할 일 삭제 함수
  const deleteTask = (id) => {
    setTodos((prevTodos) => prevTodos.filter((todo) => todo.id !== id)); // 필터링하여 새 배열로 상태 업데이트
  };

  // 입력 필드 변경 핸들러
  const handleInputChange = (e) => {
    setNewTask(e.target.value); // 입력 필드의 값 상태 업데이트
  };
