...
// React.memo : 함수를 React.memo로 래핑되면, React는 컴포넌트를 랜더링 하고 결과를 메모라이징 할때 사용
const ButtonA = React.memo(({ onClick }) => {
  console.log("props 변경으로 인해 ButtonA 랜더링");
  return <button onClick={onClick}>useCallback 미사용</button>;
});

// useCallback을 사용한 버튼
const ButtonB = React.memo(({ onClick }) => {
  console.log("props 변경으로 인해 ButtonB 랜더링");
  return <button onClick={onClick}>useCallback 사용</button>;
});

// value가 변경되면 ButtonA는 렌더링 되지만, ButtonB는 랜더링 되지 않음
...
export default function UseCallBack() {
  ...
  // useCallback을 사용하지 않는 함수
  const handleClickA = () => {
    console.log("handleClickA 호출");
    setCount((c) => c + 1);
  };

  // useCallback을 사용하는 함수
  ...
  return (
    <>
      ...
      <ButtonA onClick={handleClickA} className="border-1" />
      <ButtonB onClick={handleClickB} className="border-1" />
    </>
  );
}
