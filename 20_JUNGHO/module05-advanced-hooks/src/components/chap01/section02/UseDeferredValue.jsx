...
const SlowList = ({ text }) => {
  ...
  return <ul style={{ maxHeight: "200px", overflowY: "auto", marginTop: "10px" }}>{items}</ul>;
};
...
export default function UseDeferredValue() {
  ...
  return (
    <>
      ...
      <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="텍스트 입력 ㄱ" />
      <p>즉시 입력값 : {input}</p>
      ...
      <p>
        입력창의 텍스트를 빠르게 입력하면, "즉시 입력값"은 바로 반영되고 입력창은 반응성을 유지한다. 반면, "지연된
        입력값"과 그에 기반하는 리스트는 약간의 지연 후에 업데이트 된다. 이는 useDeferredValue가 무거운
